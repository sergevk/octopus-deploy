...
#    options installer_options
    options tentacle_installer_options
    provider Chef::Provider::Package::Windows

    not_if { ::File.exist?(tentacle_install_location) }
  end
...
action :configure do
  ...
  create_instance = powershell_script "create-instance-#{instance}" do
    ...
    EOH
    ### If instance not created - next will be errors
...
#  new_resource.updated_by_last_action(actions_updated?([firewall, install, create_home_dir, generate_cert, create_instance, configure, service]))
  new_resource.updated_by_last_action(actions_updated?([firewall, create_home_dir, generate_cert, create_instance, configure, service]))
end
